...
A Redmine 7 theme — a modern SaaS board — cool blue-grey neutrals, roomy rows, a card-like issue list. This is the **classic** variant: it keeps Redmine's own top and project navigation and builds no rail.
...
```
cp -r atlas-classic /path/to/redmine/themes/
```

Then pick **Atlas Classic** under *Administration → Settings → Display → Theme*.

Requires Redmine 7.0.

## Navigation variants

Atlas ships in two variants. They are the same design — same palette, same type,
same components — and differ only in where navigation lives:

| Folder | Navigation |
| --- | --- |
| [`atlas/`](../atlas/) | **Rail.** A collapsible left rail carries navigation. Redmine's top and project bars are hidden above 900px, because the rail already holds exactly those links and showing both repeats every item and costs two rows of vertical space. |
| [`atlas-classic/`](../atlas-classic/) | **Classic.** No rail. Redmine's global bar and project tabs stay where they are. |
...
asks Google Fonts for **Public Sans**, so a page load contacts
