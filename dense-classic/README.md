...
A Redmine 7 theme — maximum information density — small type, tight rows, a rust accent. This is the **classic** variant: it keeps Redmine's own top and project navigation and builds no rail.
...
```
cp -r dense-classic /path/to/redmine/themes/
```

Then pick **Dense Classic** under *Administration → Settings → Display → Theme*.
